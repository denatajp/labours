<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Daftar Akun</title>
    <link href="/css/styleLogin.css" rel="stylesheet" type="text/css">
</head>

<body>
    <nav>
        <div class="wrapper">
            <div class="logo">
                <a href="/"><img style="float:left; width: 170px; margin-top:14px;" href="/home" src="/logo/logo.png" alt="">
            </div>
            <div class="menu">
                <ul>
                    <li><a href="login" class="btn-sgnUp">Login</a></li>
                </ul>
            </div>
        </div>
    </nav>
    <div class="container">
        <div class="signupsuccess">
            <img src="/logo/pending.png" style="width: 200px;" alt="">
            <h1>Belum Bisa Login</h1>
            <h4>Akun anda belum diperiksa dan diverifikasi oleh admin. Mohon menunggu beberapa waktu</h4>
            <button onfocus="window.location.href = '/login';">Back</button>
        </div>
    </div>
</body>

</html>